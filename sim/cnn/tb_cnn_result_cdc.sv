...
// 验证CNN结果从摄像头时钟域进入HDMI像素时钟域时，toggle和ASCII数据保持一致。
module tb_cnn_result_cdc;
    ...
    initial begin
        ...
        if (error_count == 0)
            $display("[RESULT] CNN RESULT CDC TEST PASSED");
        else
            $display("[RESULT] CNN RESULT CDC FAILURES=%0d", error_count);
        $finish;
    end
